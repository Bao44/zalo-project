...
const messageController = {
  ...
  async sendMessage(req, res) {
    try {
      ...
      if (!conversationId || !senderId) {
        return res
          .status(400)
          .json({ error: "conversationId và senderId là bắt buộc" });
      }
      ...
      if (!conversation) {
        if (!receiverId) {
          return res
            .status(400)
            .json({ error: "receiverId là bắt buộc khi tạo conversation mới" });
        }
      ...
      if (attachments && attachments.length > 0) {
        const uploadPromises = attachments.map(async (attachment) => {
          ...
          const result = await cloudinary.uploader.upload(
            `data:image/png;base64,${attachment.fileUri}`,
            {
      ...
      if (file) {
        ...
        const populatedFileMessage = await Message.findById(
          savedFileMessage._id
        )
          .populate("senderId", "name avatar")
          .populate({
            path: "replyTo",
      ...
      if (media) {
        ...
        const populatedVideoMessage = await Message.findById(
          savedVideoMessage._id
        )
          .populate("senderId", "name avatar")
          .populate({
            path: "replyTo",
      ...
      for (const memberId of conversation.members) {
        ...
        if (memberSocketIdWeb) {
          io.to(memberSocketIdWeb).emit("newConversation", conversationUpdate);
        }
      }

      // Trả về tin nhắn cuối cùng hoặc danh sách tin nhắn (tùy yêu cầu)
      const lastPopulatedMessage = await Message.findById(
        savedMessages[savedMessages.length - 1]._id
      )
        .populate("senderId", "name avatar")
        .populate({
          path: "replyTo",
  ...
  async addUserSeen(req, res) {
    ...
    if (!conversationId || !userId) {
      return res
        .status(400)
        .json({ error: "conversationId và userId là bắt buộc" });
    }
    try {
      ...
      if (updatedMessages.modifiedCount === 0) {
        return res
          .status(404)
          .json({ error: "Không có tin nhắn nào để cập nhật" });
      }
      ...
      res
        .status(200)
        .json({ message: "Cập nhật trạng thái đã xem thành công" });
    } catch (error) {
  ...
  async deleteMessage(req, res) {
    ...
    try {
      ...
      if (message.removed.includes(userId)) {
        return res
          .status(400)
          .json({ error: "Người dùng đã xóa tin nhắn này rồi" });
      }
  ...
  async undoDeleteMessage(req, res) {
    ...
    try {
      ...
      if (message.senderId.toString() !== userId) {
        return res
          .status(403)
          .json({ error: "Bạn không có quyền thu hồi tin nhắn này" });
      }
      ...
      if (message.revoked) {
        return res
          .status(400)
          .json({ error: "Tin nhắn đã được thu hồi trước đó" });
      }
  ...
  async findPreviousMessage(req, res) {
    ...
    try {
      ...
      if (!previousMessage) {
        console.log(
          "Không có tin nhắn nào trước tin nhắn này trong cuộc hội thoại."
        );
        return null;
      }
  ...
  async likeMessage(req, res) {
    ...
    try {
              ...
              like: {
                $cond: {
                  ...
                  then: {
                    ...
                    $concatArrays: ["$like", [{ userId, totalLike: 1 }]],
                  },
                },
              },
            },
      ...
      const message = await Message.findByIdAndUpdate(messageId, updateQuery, {
        new: true,
      })
        .populate("senderId", "name avatar")
        .populate({
      ...
      return res.status(200).json({
        message:
          likeStatus === "like"
            ? "Thích tin nhắn thành công"
            : "Bỏ thích tin nhắn thành công",
        data: {
          messageId: message._id,
          like: message.like,
        },
